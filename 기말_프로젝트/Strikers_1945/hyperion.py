...
class Hyperion():
	...
	def __init__(self,x, y):
		...
		if Hyperion.image == None:
			Hyperion.image = load.image('res/Monster_4.png')
	...
	def update(self):
		...
		if self.bulletTerm >= 0.15:
			...
			Hbullet = hyperionBullet(self.x, self.y + 20, 0)
			...
			Hbullet3 = hyperionBullet.hyperionBullet(self.x - 100, self.y , 2)
			gfw.world.add(gfw.layer.Bullet, Hbullet3)

		for Mb in gfw.world.objects_at(gfw.layer.MonsterBullet):
			if (self.x - 120 <  Mb.x < self.x < self.x + 120) and (self.y - 60 < Mb.y < self.y + 60):
				Mg.isdead = True
				if Mb.index == 2:
