...
class LeftPlane1:
	...
	def update(self):
		if self.isDead or self.hp <= 0:
			...
			Eft1 = Effect.Effect(self.x + random.randint(-20, 20), self.y + random.randint(-20, 20), 128, 128, 200, 200, 9, 1)
			gfw.world.add(gfw.layer.Effect, Eft1)
			self.remove()
...
class RightPlane1:
	...
	def update(self):
		if self.isDead or self.hp <= 0:
			UI.Score().Add_Score(random.randint(100, 150))
			Eft2 = Effect.Effect(self.x + random.randint(-20, 20), self.y + random.randint(-20, 20), 128, 128, 200,200,9,1)
			gfw.world.add(gfw.layer.Effect, Eft2)
			self.remove()
		self.BTerm = (self.BTerm + gfw.delta_time * 3) % 3
